...
return new class extends Migration
{
    public function up(): void
    {
        if (DB::getDriverName() === 'sqlite') {
            ...
            // MySQL: standard schema builder for columns addition + alter status enum
            Schema::table('permohonan', function (Blueprint $table) {
                if (!Schema::hasColumn('permohonan', 'tanggal_selesai_kunjungan')) {
                    $table->dateTime('tanggal_selesai_kunjungan')->nullable();
                }
                if (!Schema::hasColumn('permohonan', 'ringkasan_pdf_path')) {
                    $table->text('ringkasan_pdf_path')->nullable();
                }
                if (!Schema::hasColumn('permohonan', 'ringkasan_uploaded_at')) {
                    $table->dateTime('ringkasan_uploaded_at')->nullable();
                }
                if (!Schema::hasColumn('permohonan', 'ringkasan_sent_at')) {
                    $table->dateTime('ringkasan_sent_at')->nullable();
                }
                if (!Schema::hasColumn('permohonan', 'ringkasan_sent_by')) {
                    $table->string('ringkasan_sent_by', 200)->nullable();
                }
            });

            DB::statement("ALTER TABLE permohonan MODIFY COLUMN status ENUM('Pending', 'Disetujui', 'Ditolak', 'Revisi', 'Selesai', 'Ringkasan_Terkirim') NOT NULL DEFAULT 'Pending'");
        }
    }
